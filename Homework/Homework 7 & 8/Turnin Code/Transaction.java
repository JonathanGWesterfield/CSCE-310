...
public class Transaction {
    ...
    * managers. This constructor depends on the file, log, and buffer managers that
    * it gets from the class {@link simpledb.server.SimpleDB}. Those objects are
    * created during system initialization. Thus this constructor cannot be called
    * until either {@link simpledb.server.SimpleDB#init(String)} or
    * {@link simpledb.server.SimpleDB#initFileLogAndBufferMgr(String)} or is called
    * first.
    */
   public Transaction() {
      txnum = nextTxNumber();
   ...
   /**
    * Commits the current transaction. Flushes all modified buffers (and their log
    * records), writes and flushes a commit record to the log, releases all locks,
   ...
   public void commit() {
      ...
      System.out.println(SimpleDB.fileMgr().blockreads() + " blocks read");
      System.out.println(SimpleDB.fileMgr().blockwrites() + " blocks written");
      SimpleDB.fileMgr().resetCounts();

   }

   /**
    * Rolls back the current transaction. Undoes any modified values, flushes those
    * buffers, writes and flushes a rollback record to the log, releases all locks,
    * and unpins any pinned buffers.
    */
   ...
   /**
    * Stores an integer at the specified offset of the specified block. The method
    * first obtains an XLock on the block. It then reads the current value at that
    * offset, puts it into an update log record, and writes that record to the log.
   ...
   /**
    * Returns the number of blocks in the specified file. This method first obtains
    * an SLock on the "end of the file", before asking the file manager to return
    * the file size.
    * 
   ...
   /**
    * Appends a new block to the end of the specified file and returns a reference
    * to it. This method first obtains an XLock on the "end of the file", before
