...
public class LogMgr implements Iterable<BasicLogRecord> {
   ...
   /**
    * Used to hash a block so we can see if there have been any changes to the
    * block. This should prevent unnecessary flushing.
    * 
    * @return
    */
   private int hashIt() {
      currentBlkHash = Objects.hash(currentblk);
      return currentBlkHash;
   }

   /**
    * Writes the current page to the log file. Only writes to log if the block has
    * had any modification done to it. If there were modifications, then the hash
    * of the object would be different from our currentBlkHash.
    */
   private void flush() {
      if (!(currentBlkHash == Objects.hash(currentBlk))) {
         mypage.write(currentblk);
         System.out.println("Acutally Flushed");
      } else {
   ...
   private void finalizeRecord() {
      ...
      currentpos += INT_SIZE;
      hashIt(); // hash the block since we just added a record
   }
